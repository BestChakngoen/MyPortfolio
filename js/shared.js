...
window.PortfolioApp.AdminControls = ({ isEditing, setIsEditing, onSave, onCancel, user, onLogin, onLogout, isOwner }) => {
    ...
    // Check if we are in public view mode (has uid in URL)
    const isPublicView = new URLSearchParams(window.location.search).has('uid');

    // Determine visibility logic:
    // 1. If 'isOwner' is true, ALWAYS show controls (User is logged in and viewing their own page).
    // 2. If NO user is logged in AND it is NOT a public shared link (Home page), show Login button.
    // 3. Otherwise (Visitor on shared link, or Logged in user viewing someone else's link), HIDE controls.
    const showAuthButton = isOwner || (!user && !isPublicView);
    ...
    return (
        <>
            {/* Toggle / Login Button - Conditionally Rendered */}
            {showAuthButton && (
            ...
            {/* Login Status Panel - Show only if user is logged in */}
            {user && !isEditing && (
                <div className="fixed bottom-4 right-20 z-50 bg-gray-800/90 backdrop-blur px-4 py-2 rounded-full border border-gray-700 flex items-center gap-3 animate-fade-in">
                    ...
                    {/* Share Button (Show only for Owner) */}
                    {isOwner && (
            ...
            {/* Edit Actions Panel - Protected by isOwner implicitly via showAuthButton logic but good to double check */}
            {isEditing && isOwner && (
